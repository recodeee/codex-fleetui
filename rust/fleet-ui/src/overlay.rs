...
//!
//! Shared geometry/chrome primitives used by every overlay surface
//! (`spotlight_overlay`, `session_switcher_overlay`, `context_menu`, …).
//! Three pieces:
//!
...
//! 3. [`card_shadow`] — paints the standard right-edge + bottom-edge
//!    drop shadow band that overlays use to lift off the background.
//!
//! NOTE: the Spotlight overlay used to live here as well (a substring-
//! filter sibling of [`crate::spotlight_overlay`]). It was removed when
//! the two implementations were consolidated onto the fuzzy
//! [`crate::spotlight_overlay::Spotlight`]; see `SHARED_SPOTLIGHT_ITEMS`
//! there for the canonical command catalogue.
